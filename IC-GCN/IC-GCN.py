...
#注意 如果需要运行小数据集：n_fold被改成2，原本20！！！ ssl_ratio改成0，原本0.1！！！

#新增加参数ssl_reg，最开始0.5
#新增加参数ssl_temp，最开始0.5
#新增加参数ssl_ratio，最开始0.1

#相比IMP_GCN_final，改动为：无

recordFileName = 'recordFile_IMPGCNfinal.txt'
...
class IMP_GCN(object):
    ...
    def create_adj_mat(self):
        ...
        R = sp.csr_matrix(
            (np.ones_like(self.users_ALL, dtype=np.float32), (self.users_ALL, self.items_ALL)),
    ...
    def __init__(self, data_config, pretrain_data):
        ...
        #LinChuan
        self.users_ALL = self.norm_adj[0:self.n_users,self.n_users:].todense().nonzero()[0]
        self.items_ALL = self.norm_adj[0:self.n_users,self.n_users:].todense().nonzero()[1]
        self.norm_adj_sub1=self.create_adj_mat()
        self.norm_adj_sub2=self.create_adj_mat()
        ...
        *********************************************************
        ...
        #LinChuan
        self.ua_embeddings_sub1, self.ia_embeddings_sub1, self.A_fold_hat_group_filter_sub1, self.user_group_embeddings_sub1 = self._create_imp_gcn_embed_sub1()
        self.ua_embeddings_sub2, self.ia_embeddings_sub2, self.A_fold_hat_group_filter_sub2, self.user_group_embeddings_sub2 = self._create_imp_gcn_embed_sub2()
    ...
    def _create_imp_gcn_embed_sub1(self):
        #self.norm_adj  ->  #self.norm_adj_sub1
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj_sub1)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj_sub1)
        ...
        user_group_embeddings = tf.concat([u_group_embeddings, i_group_embeddings], axis=0)
        # Matrix mask
        A_fold_hat_group, A_fold_hat_group_filter = self._split_A_hat_group(self.norm_adj_sub1, user_group_embeddings)
    ...
    def _create_imp_gcn_embed_sub2(self):
        #self.norm_adj_sub1 #self.norm_adj_sub2
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj_sub2)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj_sub2)
        ...
        user_group_embeddings = tf.concat([u_group_embeddings, i_group_embeddings], axis=0)
        # Matrix mask
        A_fold_hat_group, A_fold_hat_group_filter = self._split_A_hat_group(self.norm_adj_sub2, user_group_embeddings)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    print("ssl ratio:", model.ssl_ratio)
    print("ssl temp:", model.ssl_temp)
    print("ssl reg:", model.ssl_reg)
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            # #LinChuan
            # model.users_version_not_placeholder=users
            # model.pos_items_version_not_placeholder=pos_items
            # model.neg_items_version_not_placeholder=neg_items

            _, batch_loss, batch_mf_loss, batch_emb_loss, batch_reg_loss, batch_ssl_loss, group_filter, user_group_embeddings = sess.run([model.opt, model.loss, model.mf_loss, model.emb_loss, model.reg_loss, model.ssl_loss, model.A_fold_hat_group_filter, model.user_group_embeddings],
        ...
        if (epoch + 1) % 10 != 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                perf_str = 'Epoch %d [%.1fs]: train==[loss %.5f = mf_loss %.5f + emb_loss %.5f + ssl_loss %.5f + reg_loss %.5f]' % (
                    epoch, time() - t1, loss, mf_loss, emb_loss, ssl_loss, reg_loss)
                print(perf_str)
                with open(recordFileName, 'a') as file:
                    file.write(perf_str)
                    file.write("\n")
            continue
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f + %.5f], recall=[at 1 %.5f, at K %.5f], ' \
                       'precision=[at 1 %.5f, at K %.5f], hit=[at 1 %.5f, at K %.5f], ndcg=[at 1 %.5f, at K %.5f]' % \
                       (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, ssl_loss, reg_loss, ret['recall'][0], ret['recall'][-1],
    ...
    print(final_perf)
    with open(recordFileName, 'a') as file:
        file.write(final_perf)
        file.write("\n")
    with open(recordFileName, 'a') as file:
        file.write("Project normally over!!!!!!")
        file.write("\n")
