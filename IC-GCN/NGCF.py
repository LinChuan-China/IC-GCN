...
recordFileName = 'recordFile_IMPGCNoriginal.txt'
class IMP_GCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.ua_embeddings, self.ia_embeddings = self._create_NGCF_embed()#LinChuan!!!!!
        """
        *********************************************************
    ...
    def _init_weights(self):
        ...
        for k in range(self.n_layers):
            all_weights['ngcf_W_gc_%d' %k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k+1]]), name='ngcfW_gc_%d' % k)
            all_weights['ngcf_b_gc_%d' %k] = tf.Variable(
                initializer([1, self.weight_size_list[k+1]]), name='ngcf_b_gc_%d' % k)

            all_weights['ngcf_W_bi_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]]), name='ngcf_W_bi_%d' % k)
            all_weights['ngcf_b_bi_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[k + 1]]), name='ngcf_b_bi_%d' % k)
        return all_weights
    ...
    def _create_NGCF_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            sum_embeddings = tf.nn.leaky_relu(
                tf.matmul(side_embeddings, self.weights['ngcf_W_gc_%d' % k]) + self.weights['ngcf_b_gc_%d' % k])

            # bi messages of neighbors.
            bi_embeddings = tf.multiply(ego_embeddings, side_embeddings)
            # transformed bi messages of neighbors.
            bi_embeddings = tf.nn.leaky_relu(
                tf.matmul(bi_embeddings, self.weights['ngcf_W_bi_%d' % k]) + self.weights['ngcf_b_bi_%d' % k])
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % 10 != 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                perf_str = 'Epoch %d [%.1fs]: train==[loss %.5f = mf_loss %.5f + emb_loss %.5f  + reg_loss %.5f]' % (
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], recall=[at 1 %.5f, at K %.5f], ' \
